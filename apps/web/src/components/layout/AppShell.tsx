...
export default function AppShell({ children }: { children?: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-bg pb-20 md:pb-0 md:pl-60">
      <aside className="hidden md:flex fixed inset-y-0 left-0 w-60 bg-surface border-r border-border flex-col p-4">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium ${
                  isActive ? 'bg-accent text-white' : 'text-text hover:bg-surface-2'
                }`
              }
            >
              <span>{n.icon}</span>
        ...
        <button
          onClick={signOut}
          className="text-sm text-muted hover:text-text text-left px-3 py-2"
        >
          Sair
        </button>
      </aside>

      <nav className="md:hidden fixed inset-x-0 bottom-0 bg-surface border-t border-border flex overflow-x-auto">
        {NAV.map((n) => (
          <NavLink
            key={n.to}
            to={n.to}
